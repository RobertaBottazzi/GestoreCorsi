...
public class FXMLController {
    ...
    void corsiPerPeriodo(ActionEvent event) {
    	...
    	for(Corso c: corsi) {
    		sb.append(String.format("%-8s ", c.getCodins())); //colonna con 8 spazi allineata a sx (-) dove inserisco c.getCodins()
    		sb.append(String.format("%-4d ", c.getCrediti()));
    		sb.append(String.format("%-50s ", c.getNome()));
    		sb.append(String.format("%-4d\n", c.getPd()));
    		//potevo anche fare tutto su una riga sola
    ...
    public void setModel(Model model) {
    	this.model = model;
    	txtRisultato.setStyle("-fx-font-family: monospace"); //da prendere per buona per poter utilizzare i placeholder dello string format, basta farlo una volta sola in qualsiasi punto del programma.
    }
